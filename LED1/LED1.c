...
#use rs232(baud=9600,parity=N,xmit=U1TX,rcv=U1RX,bits=8,stream=PORT1)

#pin_select U2TX = PIN_D3
#pin_select U2RX = 	PIN_D2
#use rs232(baud=9600,parity=N,xmit=U2RX,rcv=U2RX,bits=8,stream=PORT2)

#pin_select U3TX = PIN_E5
#pin_select U3RX = PIN_E4
#use rs232(baud=9600,parity=N,xmit=U3TX,rcv=U3RX,bits=8,stream=PORT3)

#pin_select U4TX = PIN_E3
#pin_select U4RX = PIN_F2
#use rs232(baud=9600,parity=N,xmit=U4TX,rcv=U4RX,bits=8,stream=PORT4)
```
...
void main()
{
    printf("LED1 test start\r\n");
    output_high(DIO_3);
    delay_ms(500);
    output_low(DIO_3);
    delay_ms(500);
    printf("LED1 test end\r\n");
}